    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float edge = saturate(expandedA * _GlowSoftness);
                float outsideMask = saturate(edge * (1.0 - baseCol.a));
                
                float4 glow = _GlowColor * (outsideMask * _GlowIntensity * pulse);
                float4 col = baseCol;
                col.rgb += glow.rgb;
                col.a = saturate(max(baseCol.a, glow.a));
                return col;
            }
            ENDHLSL
        }
